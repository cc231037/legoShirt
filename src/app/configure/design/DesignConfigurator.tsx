...
import { COLORS, FINISHES, MATERIALS, SERIES } from "@/validators/option-validators"
...
const DesignConfigurator = ({
    ...
    const [options, setOptions] = useState<{
        ...
        series: (typeof SERIES.options)[number]
        material: (typeof MATERIALS.options)[number]
        finish: (typeof FINISHES.options)[number]
    }>({
        color: COLORS[0],
        series: SERIES.options[0],
    ...
    return (
        <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
            <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
                <div className="relative w-96 bg-opacity-50 pointer-events-none aspect-[2000/2000]">
                    <AspectRatio ref={phoneCaseRef}
                    ratio={2000 / 2000} className="pointer-events-none relative z-50 aspect-[2000/2000] w-full">
                        <NextImage fill alt='phone image' src={`/torso-template-${options.color.value}.png`}
                            className="pointer-events-none z-50 select-none" />
                    </AspectRatio>
                    <div className="absolute z-40 inset-0 shadow-[0_0_0_99999px_rgba(235,235,235,0.8)]" />
                    <div className={cn(`absolute inset-0 bg-${options.color.tw}`)}
                        style={{
                            clipPath: 'polygon(28% 28%, 72% 28%, 81% 78%, 19% 78%)',
                            borderRadius: '32px'
                        }}
                    />
                                            ...
                                            {SERIES.options.map((series) => (
                                                <DropdownMenuItem
                                                    ...
                                                    className={cn(
                                                        ...
                                                        {
                                                            'bg-zinc-100':
                                                                series.label === options.series.label,
                                                        }
                                                    )}
                                                    onClick={() => {
